...
CREATE TABLE IF NOT EXISTS team_engagement_metrics (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, COALESCE(department_id, '00000000-0000-0000-0000-000000000000'), 
                   COALESCE(team_id, '00000000-0000-0000-0000-000000000000'), metric_date)
);
...
SELECT 
    ...
    ep.points_available,
    (SELECT AVG(engagement_score) FROM engagement_scores WHERE employee_id = e.id) AS avg_engagement_score,
...
SELECT 
    ...
    tem.turnover_rate,
    tem.metric_date,
    (SELECT COUNT(*) FROM employees e WHERE e.primary_department = tem.department_id AND e.deleted_at IS NULL) AS total_employees,
    (SELECT AVG(mood_score) FROM mood_tracking mt 
     JOIN employees e ON mt.employee_id = e.id 
     WHERE e.primary_department = tem.department_id AND mt.track_date = tem.metric_date) AS avg_mood_score
FROM team_engagement_metrics tem
LEFT JOIN departments d ON tem.department_id = d.id
LEFT JOIN teams t ON tem.team_id = t.id;
...
SELECT 
    ...
    RANK() OVER (PARTITION BY tenant_id ORDER BY SUM(points_awarded) DESC) AS rank
FROM recognitions r
JOIN employees e ON r.receiver_id = e.id
LEFT JOIN departments d ON e.primary_department = d.id
WHERE r.status = 'awarded' AND r.deleted_at IS NULL
GROUP BY tenant_id, receiver_id, e.first_name, e.last_name, d.name;
